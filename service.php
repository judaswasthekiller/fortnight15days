...
*/
$dt= date("Y-m-d");
$sql ="insert into service(custid,carname,carno,date,address,city,landmark,zipcode,status) values('$_SESSION[custid]','$_POST[vehname]','$_POST[carno]','$dt','$_POST[address]','$_POST[city]','$_POST[landmark]','$_POST[zip]','Pending')";
...
if(isset($_POST["submit"]))
{
	echo "<h4><center>car service request sent successfully...</center></h4>";
}
else
{
			?>
          <table width="563" border="0">
			  ...
			  <tr>
			    <th height="36" scope="row">car No
                  <label for="lname2"></label></th>
			    <td><label for="email"></label>
			      <label for="model"></label>
			      <label for="carno"></label>
		        <!--input type="text" name="carno" id="carno"  onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" size="35" /></td-->
				<input type="text" name="carno" id="carno"  onKeypress="" size="35" /></td>
		      </tr>
			  <tr>
			  ...
			  <tr>
			    <th scope="row">&nbsp;</th>
			    <td><input type="submit" name="submit" id="submit" value="Request to car service" /></td>
		      </tr>
          </table>
          <?php
}
