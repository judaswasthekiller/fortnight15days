...
$sql ="insert into carstore(empid,vehname,model,brand,images,estprice,description,status) 
...
if($img != "")
{
$sql ="UPDATE carstore set empid='$_SESSION[empid]',vehname='$_POST[vehname]',images='$img',model='$_POST[model]',brand='$_POST[brand]',estprice='$_POST[estprice]',description='$_POST[description]',status='$_POST[status]' where carid='$_GET[carid]'";
}
else
{
$sql ="UPDATE carstore set empid='$_SESSION[empid]',vehname='$_POST[vehname]',model='$_POST[model]',brand='$_POST[brand]',estprice='$_POST[estprice]',description='$_POST[description]',status='$_POST[status]' where carid='$_GET[carid]'";
}
if (!mysql_query($sql,$con))
  {
  die('Error: ' . mysql_error());
...
?>
		...
		<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onlclick="return validate()">		
			<table width="325", border="0">
			  ...
			  <tr>
			    <th scope="row">&nbsp;</th>
			    <td><?php if(isset($_GET['carid'])) { ?> <input type="submit" name="update" id="submit" value="Update car" /> <?php } else { ?> <input type="submit" name="submit" id="submit" value="Register" /><?php } ?></td>
		      </tr>
          </table>
          <?php
			}
