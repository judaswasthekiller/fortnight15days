...
$sql ="insert into testdrive(carid,custid,date,time,comments,status) values('$_POST[carid]','$_SESSION[custid]','$newDate','$tim','$_POST[comment]','Pending')";
...
if(isset($_POST["update"]))
{
 ...
 $newDate = date("Y-m-d", strtotime($originalDate));
	 $tim = $_POST[hour].":". $_POST[minute].":00";
$sql ="update testdrive set carid='$_POST[carid]',custid='$_SESSION[custid]',date='$newDate',time='$tim',comments='$_POST[comment]' where bookingid='$_GET[testid]'";
 ...
 ?>   
          ...
          <form id="form1" name="form1" method="post" action="" onsubmit="return validate()">		
			...
			?>
			  ...
			  ?>
			  <tr>
			    <th height="37" scope="row">car name
			      <label for="vehname"></label></th>
			    <td>
                <?php if(isset($_GET['caridnum']))
			  {
			  $res = mysql_query("select vehname from carstore where carid='$_GET[caridnum]'");
			  $rows = mysql_fetch_array($res);
			  ?><input name="carname" type="text" value="<?php echo $rows['vehname']; ?>" /><input type="hidden" name="carid" value="<?php echo $_GET['caridnum']; ?>" />
              <?php } else { ?>
              <select name="carid" id="bstatus3">
                <?php
				$sql = mysql_query("select vehname,carid from carstore");
				while($row = mysql_fetch_array($sql))
				{
				if($row['carid'] == $res['carid'])
				{
				?>
                <option value="<?php echo $row['carid']; ?>" selected="selected"><?php echo $row['vehname']; ?></option>
		        <?php }else{ ?>
                <option value="<?php echo $row['carid']; ?>"><?php echo $row['vehname']; ?></option>
                <?php
				}
				}
				?>
